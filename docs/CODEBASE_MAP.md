...
- **Declaration context:** clicking a declaration item with call-graph data in the interior panel switches the flowchart to declaration context, showing the selected declaration as a center node with outgoing calls (left lane) and incoming callers (right lane). Each callee/caller node is color-coded by declaration kind. Navigable declarations (those with call-graph entries) can be clicked to chain into further declaration contexts. A breadcrumb trail at the top of the flowchart provides module-name links and a "Module Context" button to return to the module-level flowchart. Declaration context is persisted in the URL via a `decl` parameter. When a module is selected via search, the flowchart automatically returns to module context. Caller lookups use a precomputed reverse graph index for O(1) performance. When a lane exceeds 12 entries, the first 10 are shown with a "+N more" summary node to keep the flowchart readable.
...
- **Interior declaration explorer:** the flow chart context now exposes all interior Lean declaration kinds via three dropdowns:
  ...
  - Declarations with call-graph data display a clickable name (entering declaration context) alongside a compact "src" link for direct GitHub source access. Declarations without call-graph data continue to link directly to source.
