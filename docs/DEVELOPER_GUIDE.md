...
Written **compact** (no indentation): at ~1.6 MB it is the dominant payload on
map.html, and indenting it costs roughly 100 KB of gzipped transfer for a
generated file no one reads as text. `site-data.json` and `execution-traces.json`
stay indented.
...
Synced by `scripts/sync-trace-data.mjs`; validated by `scripts/validate-traces.mjs`.

## 7) Data-generation scripts (`scripts/`)

### `scripts/sync-site-data.mjs`
Builds `data/site-data.json` with every landing-page statistic projected from the
canonical upstream `docs/codebase_map.json` artifact — the same file from which
seLe4n's own README table is generated. `canonicalMetricsIssues()` gates the
write: a missing key aborts the sync with the metric it feeds, so an upstream
schema change surfaces as a red build instead of a plausible-looking wrong
number.

Nothing outside the artifact contributes a published figure. The one exception
is the repository's non-Lean inventory (`scripts`, `docs`), which the artifact
does not describe; those are exact file counts read from the git tree.

Network shape: one raw file download plus one blobless shallow clone (~200 KB,
trees only). No REST calls, so no anonymous rate limit, no token, and no
exposure to the tree endpoint's `truncated` cliff.
...
- resolving the upstream tip over the git protocol and downloading a single source archive (no per-file REST calls, so it stays inside anonymous rate limits),
- enumerating `SeLe4n/**/*.lean` files from the extracted tree,
- parsing imports and declarations locally,
...
Trace schema validation and the deterministic fold engine (`reconstructState`/`scenarioStates`) shared by `validate-traces.mjs`, `sync-trace-data.mjs`, and the Simulator tests.
