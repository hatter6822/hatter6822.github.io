...
- handles keyboard navigation, search, reset, and URL-state synchronization (including `decl` parameter for declaration context persistence). The module-search bar is context-aware, displaying `ModuleName › DeclarationName` and updating the label in declaration context. The `flowchart-wrap` `aria-label` updates dynamically per context. Supports dot-append declaration search (e.g., `SeLe4n.Kernel.API.apiInvariantBundle`) via `declarationSearchMatch()`, which progressively tries shorter dot-separated module prefixes and matches the remaining suffix against interior symbols and `declarationIndex`. Exact matches navigate immediately; partial matches appear as styled suggestions with `data-declaration` attributes. The search flow integrates `tryDeclarationSearch` as a fallback when no module match is found.
