...
Rewrites `index.html`'s static fallback values (mapped `data-live` spans, JSON-LD version, snapshot `<time>` stamp) from `data/site-data.json` via `scripts/lib/static-values.mjs`. Idempotent; run after `sync-site-data.mjs`. The committed tree must stay in sync — `static-values.test.mjs` fails otherwise.
...
The `data/site-data.json` → `index.html` static-fallback mapping used by `scripts/apply-static-values.mjs` and the weekly sync workflow. Single source of truth for which spans are rewritten.
...
- `static-values.test.mjs`: pins the static-fallback rewriter mapping and asserts the committed `index.html` matches `data/site-data.json`.
