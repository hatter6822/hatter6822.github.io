...
- Rust inventory validation (`validateMapDataObject`): the optional `rust` block must name files the snapshot's own `files[]` lists, use known item kinds, visibilities and positive lines, carry per-crate totals equal to the per-file sums of production items, and have `testItems` counts that match the items flagged `test: true`; a malformed block is rejected with a message naming the field
...
- Confirm the Rust section shows the dependency strip (`sys → abi → types`, HAL standalone) and four crate cards; that `sele4n-types` and `sele4n-sys` read `none` in the `unsafe` cell, `sele4n-abi` reads `2 fn · 1 blocks · 0 impl allowed by exception` with a yellow strip node, and `sele4n-hal` lists 118 sites with the strip agreeing; that the three `deny` crates state `#![deny(unsafe_code)]` in their facts line; and that opening a file renders its item list with line links.
...
- Confirm a crate's "Show N test items" toggle lists the flagged test items with a `test` tag, keeps the files that were open, and that integration-test files read "test items hidden" until it is on.
- Confirm `Main`'s import of `SeLe4n.Testing.MainTraceHarness` renders in the external lane as "in-repo · outside production scope", not "external dependency", and that the repository inventory files `SeLe4n/Testing/**` under Tests.
