...
- Code map runtime (`map-runtime.test.mjs`): the workspace defaults to `SeLe4n.Kernel.API` and falls back to the first module only when the snapshot lacks it; `moduleSubsystem` caps at three segments; over-budget lanes group by subsystem (largest first, singletons plain, members listed nested when a group is open, flat within budget and in expanded mode); the live tree path keeps `Main.lean` and drops `tests/` and `SeLe4n/Testing/`; a canonical payload's `repository.head.commit_sha` becomes the graph commit; in-repository imports outside the scope and the library root are labelled as such; `retainInventory` keeps the tree and Rust inventory across canonical and tree refreshes with their commits; a malformed `rust` block is dropped on normalization; `rustUnsafeSummary` / `rustUnsafeDetail` keep the lint, the production sites and the test sites apart; plural fallbacks; tab selection; locale digit grouping. The 0.31.0 model: `buildRustGraph` gives one node per production source file with test targets excluded, addresses each role correctly and links parents, children and siblings; `toBridgeKey` folds case convention; `bridgeRelation` reads the four relations off the Lean kind and only then the crate stratum; `buildBridgeIndex` reproduces the FFI seam on the bundled snapshot with both readings indexing the same edges; `bridgeBandRows` mirrors direction; the scope decides node membership, defaults and the fallback on a narrowing switch, and `sanitizeModuleName` admits `::` and `-` while rejecting everything else
- Page structure (`map-toolbar.test.mjs`): the workspace grid is single-column by default and gains the sticky sidebar column only from `90rem`; the declaration explorer is a tablist; `.flowchart-svg` is `width: auto; min-width: 100%` and no rule scales it to its column; the desktop minimum flow width is 900; the default module constant and the lane-grouping functions exist; the page carries exactly one section and neither removed container remains; the scope toggle is a radiogroup whose statically active option is the one `DEFAULT_SCOPE` names; the hero stats carry the Rust module and boundary-link counts and no longer the repository file count; the removed sections leave no styles behind; the unsafe summary names production and test sites apart; target-scoped dependencies keep their cfg; the Rust graph, chart and boundary functions exist and the Rust chart reuses the shared layout
...
- Confirm switching the locale re-labels the sidebar tabs, the chart's lane labels and both legends without a reload, groups the theorem count the locale's way (`10.929` in Spanish), and pluralises count labels ("1 file", "2 files"; four forms in Ukrainian).
...
- At a phone width (390px) confirm the scope toggle spans the toolbar with three tappable options of at least 2.6rem, and that neither chart scrolls the page sideways.
...
snapshot has painted, the chart's lane labels and legend must still come out
in Spanish, which is the first-load repaint. Since 0.31.0 it also drives the
scope toggle end to end: the boundary band appears in the combined scope and
vanishes in the single-language ones, a boundary node crosses into the other
language, the Rust chart holds the same 1:1 guarantee at every width and on a
