...
**Scheduler**, **Capability**, **Memory**, and **Information-flow** scenes; later phases
deepen them (§5).
...
`cdt`, `untyped`, and `infoflow` blocks feed the Capability, Memory, and Information-flow
scenes. Later phases extend the projection further with `cnodes`, `vspace`, `services`,
and `tlb` (§5) — all additive.
...
| `data/execution-traces.json` | Bundled reference fixture (6 scenarios, 30 steps). |
...
- **Phase 1 — Vertical slice (shipped).** Schema v1, fold engine + tests + validator +
  headless runtime test + `sync-trace-data.mjs`, **all five scenes** (System, Scheduler,
  Capability, Memory, Information-flow) with tab switching, the invariant rail, inspector,
  event log, transport, deep-link URL state (incl. `scene`), the clearly-labeled sandbox,
  full chrome/i18n/theming, and a 6-scenario reference fixture (IPC call/reply,
  notification signal/wait, EDF budget preemption, capability mint/revoke, untyped
  retype/reclaim, information-flow non-interference). Honest provenance via the source
  badge + disclaimer.
...
- **Phase 4 — Memory depth + Services.** VSpace page tables / W^X / TLB / ASIDs (untyped
  retype already shipped in the Memory scene, and the information-flow lattice in its own
  scene); a service dependency-DAG lifecycle scene.
