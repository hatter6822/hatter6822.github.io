...
every transition. It offers five switchable **scenes** — **System** (CPU, run queue,
...
carving typed objects out of memory and reclaiming them on revoke), and **Information
flow** (the security-domain lattice, where the kernel blocks a leak from secret to
public until an audited declassification authorizes it). A transport bar
